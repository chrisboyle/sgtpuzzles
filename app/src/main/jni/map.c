                                   ...
                                   const char **desc, int *map)
{
    ...
    while (*p && *p != ',') {
	if (*p < 'a' || *p > 'z') {
            err = _("Unexpected character in edge list");
            goto out;
        }
	if (*p == 'z')
	    k = 25;
	...
	while (k-- > 0) {
		...
		dy = 0;
                ...
                err = _("Too much data in edge list");
                goto out;
            }
	    if (!state)
    ...
    assert(pos <= 2*wh-w-h);
    if (pos < 2*wh-w-h) {
        err = _("Too little data in edge list");
        goto out;
    }
    ...
    if (pos != n) {
        err = _("Edge list defines the wrong number of regions");
        goto out;
    }
