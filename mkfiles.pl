...
if (defined $makefiles{"android"}) {
    ...
    print
    "# Makefile for $project_name under Android (using Native Development Kit).\n".
    "#\n# This file was created by `mkfiles.pl' from the `Recipe' file.\n".
    "# DO NOT EDIT THIS FILE DIRECTLY; edit Recipe or mkfiles.pl instead.\n";
    # gcc command line option is -D not /D
    ($_ = $help) =~ s/=\/D/=-D/gs;
    ...
    &splitline("CFLAGS = -O2 -Wall -Werror -DSLOW_SYSTEM -DANDROID -DNO_PRINTING -g " .
	       ...
	       (join " ", map {"-I$dirpfx$_"} @srcdirs))." -DNDEBUG\n".
    "\n".
    ...
    foreach $d (&deps("X.o", undef, $dirpfx, "/")) {
      $oobjs = $d->{obj};
      $ddeps= join " ", @{$d->{deps}};
      print &splitline(sprintf("android/tmp/%s: %s", $oobjs, $ddeps)),
          "\n\tmkdir -p android/tmp\n";
      $deflist = join "", map { " -D$_" } @{$d->{defs}};
      print "\t\$(CC) \$(COMPAT) \$(FWHACK) \$(CFLAGS) \$(XFLAGS)$deflist" .
	  " -c \$< -o \$\@\n";
    ...
    print "\nclean:\n".
    &splitline("\trm -rf *.o *.html puzzles.hhp *.jar *.apk libpuzzles.so.unstripped android/*.apk android/tmp android/bin android/bin-stamp android/classes.dex android/lib android/src/name/boyle/chris/sgtpuzzles/R.java android/res/drawable android/assets android/res/values/arrays.xml\n");
    select STDOUT; close OUT;
}
