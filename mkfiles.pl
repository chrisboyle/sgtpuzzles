  ...
  while (scalar @objs > 0) {
    ...
    if ($groups{$i}) {
      foreach $j (@{$groups{$i}}) { unshift @objs, $j; }
              ...
              $i eq "[X]" or $i eq "[U]" or $i eq "[MX]" or
              $i eq "[GA]") and defined $prog) {
...
sub mfval($) {
    ...
    if (grep { $type eq $_ }
	("vc","vcproj","cygwin","borland","lcc","gtk","mpw","nestedvm","osx","wce","android")) {
	    return 1;
	}
...
if (defined $makefiles{"android"}) {
    $mftyp = 'android';
    $dirpfx = &dirpfx($makefiles{'android'}, "/");

    ##-- NestedVM makefile
    open OUT, ">$makefiles{'android'}"; select OUT;
    print
    "# Makefile for $project_name under Android (using NestedVM).\n".
    "#\n# This file was created by `mkfiles.pl' from the `Recipe' file.\n".
    "# DO NOT EDIT THIS FILE DIRECTLY; edit Recipe or mkfiles.pl instead.\n";
    # gcc command line option is -D not /D
    ($_ = $help) =~ s/=\/D/=-D/gs;
    ...
    "ANDROID = /opt/android-sdk-linux_x86-1.0_r1\n".
    "# You can define this path to point at your tools if you need to\n".
    "TOOLPATH = \$(NESTEDVM)/upstream/install/bin\n".
    "CC = \$(TOOLPATH)/mips-unknown-elf-gcc\n".
    &splitline("CFLAGS = -O2 -Wall -Werror -DSLOW_SYSTEM -g " .
    ...
    print "\tcp icons/$_-48d24.png android/res/drawable/${_}_48d24.png\n" for(@realgames);
    print "\tcp icons/net-48d24.png android/res/drawable/icon.png\n\n";
    ...
    foreach $p (&prognames("GA")) {
      ($prog, $type) = split ",", $p;
      $objstr = &objects($p, "X.o", undef, undef);
      print &splitline($prog . ": " . $objstr), "\n".
        "\t\$(CC) \$(${type}LDFLAGS) -o \$@ \$^ -lm\n\n";
    }
    foreach $d (&deps("X.o", undef, $dirpfx, "/")) {
      $oobjs = $d->{obj};
      $ddeps= join " ", @{$d->{deps}};
    ...
    print "\nclean:\n".
    &splitline("\trm -rf *.o android/puzzles.mips *.html *.jar *.apk android/*.apk android/bin android/bin-stamp android/classes.dex android/src/name/boyle/chris/sgtpuzzles/R.java android/res/drawable android/res/values/arrays.xml\n");
    select STDOUT; close OUT;
}
